...
class Hefftrans1363(nn.Module):
    def __init__(self):
        super(Hefftrans1363, self).__init__()
    ...
    def load_weight(self, module_1_path: str, module_2_path: str):
        ...
        self.vit.load_state_dict(torch.load(module_2_path, map_location=self.device))
    ...
    def forward(self, x: torch.Tensor):
        ...
        if x.size(0) % 13 != 0:
            raise ValueError("x: Tensor, batch_size must be multiple of 13")
        # print(x.shape)
        x = self.efficient_net(x)  # shape (batch_size, 3)
        # print(x.shape)
        x = x.view(-1, 13, 3)  # shape (batch_size // 13, 13, 3)
        # print(x.shape)
        x = self.vit(x)  # shape (batch_size // 13, 13, 3)
        # print(x.shape)
        return x
